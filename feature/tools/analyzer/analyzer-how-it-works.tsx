"use client";

import { useTranslations } from "next-intl";
import { BlurFade } from "@/components/magicui/blur-fade";

const STEP_KEYS = [
  { step: "1", title: "step1Title", description: "step1Description" },
  { step: "2", title: "step2Title", description: "step2Description" },
  { step: "3", title: "step3Title", description: "step3Description" },
] as const;

export function AnalyzerHowItWorks() {
  const t = useTranslations("analyzer");

  return (
    <section className="px-4 py-16 bg-linear-to-b from-background to-muted/20">
      <div className="container mx-auto max-w-3xl space-y-12">
        <BlurFade delay={0.1} inView>
          <div className="space-y-4 text-center">
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight">
              {t("howItWorksTitle")}
            </h2>
            <p className="text-muted-foreground max-w-xl mx-auto">
              {t("howItWorksDescription")}
            </p>
          </div>
        </BlurFade>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {STEP_KEYS.map((item, i) => (
            <BlurFade key={item.step} delay={0.2 + i * 0.1} inView>
              <div className="text-center space-y-3 p-6 rounded-xl border border-border bg-card">
                <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary/10 text-primary font-bold text-lg">
                  {item.step}
                </div>
                <h3 className="font-semibold">{t(item.title)}</h3>
                <p className="text-sm text-muted-foreground">{t(item.description)}</p>
              </div>
            </BlurFade>
          ))}
        </div>
      </div>
    </section>
  );
}
